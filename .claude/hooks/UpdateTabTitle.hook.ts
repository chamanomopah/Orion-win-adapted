 ...
 * Cross-platform support: Windows Terminal, Kitty, and other terminals
 */
function setTabTitle(title: string, state: TabState = 'normal'): void {
  try {
    ...
    if (isWindows) {
      // Windows Terminal / ConHost escape sequence
      // Using OSC 0 (set window title) and OSC 2 (set icon title)
      const osc = `\x1b]0;${truncated}\x07`;
      process.stdout.write(osc);
      console.error(`[UpdateTabTitle] Set title via Windows Terminal escape sequence`);
    } else if (isKitty) {
      ...
      // Fallback to escape codes for other Unix terminals (xterm, iTerm2, etc.)
      const escaped = truncated.replace(/'/g, "'\\''");
      try {
        // Try shell printf for better compatibility
        ...
        // Direct stdout write as fallback
        const osc = `\x1b]0;${escaped}\x07`;
        process.stdout.write(osc);
      }
    }
  } catch (err) {
