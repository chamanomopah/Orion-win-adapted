...
`;
  ...
  // Update 'current' symlink (or directory junction on Windows)
  ...
  // On Windows, create a junction. On Unix, create a symlink.
  try {
    symlinkSync(taskDirName, currentLink, PLATFORM === 'win32' ? 'junction' : undefined);
  } catch (err) {
    // If symlink/junction fails, copy as fallback
    console.error('[AutoWork] Symlink/junction failed, copying directory:', err);
    // Could implement copy fallback here if needed
  }
